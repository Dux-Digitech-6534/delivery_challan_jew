...
frappe.ui.form.on("Delivery Challan", {
	...
	set_primary_actions(frm) {
		...
		if (
			...
			frm.add_custom_button(__("Create Receipt"), () => make_delivery_challan_receipt(frm));
		}
	...
	toggle_sections(frm) {
		...
		const show_transit = has_partial_receipt(frm);

		frm.toggle_display("stock_entry_section", has_dispatch || has_receipt || has_shortage);
		frm.toggle_display("transit_warehouse", show_transit);
...
function make_delivery_challan_receipt(frm) {
	frappe.call({
		method:
...
function has_partial_receipt(frm) {
	...
	return (
		...
		(frm.doc.items || []).some((row) => flt(row.received_qty) > 0 && flt(row.pending_qty) > 0) ||
		(frm.doc.items || []).some((row) => row.row_status === "Partially Received")
	);
}
